...
export class AuthService {
	...
	registerAccount(email: string, password: string, fullName: string, companyName: string, companyId?: string) {
		if (this.valid.checkFullName(fullName))
			this.auth
				...
				.then((res) => {
					if (res.user) {
						if (companyId)
							this.db.addManager(res.user, fullName, companyName).then((manager) => {
								this.currUser.next(manager);
							});
						else
							this.db.addEmployee(res.user, fullName, companyId).then((employee) => {
